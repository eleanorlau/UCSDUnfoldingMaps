	...
	private void selectMarkerIfHover(List<Marker> markers) {
		...
		if (lastSelected == null) {
			for (Marker marker : markers) {
				boolean isInsideMarker = marker.isInside(map, mouseX, mouseY);
				if (isInsideMarker) {
		...
		if (lastClicked != null) {
			...
			if (lastClicked instanceof EarthquakeMarker) {
				for (Marker qmarker : quakeMarkers) {
					...
					for (Marker cmarker : cityMarkers) {
						if ((cmarker.getDistanceTo(lastClicked.getLocation())) > ((EarthquakeMarker) lastClicked).threatCircle()) {
			...
			if (lastClicked instanceof CityMarker) {
					...
					for (Marker qmarker : quakeMarkers) {
						if ((qmarker.getDistanceTo(lastClicked.getLocation())) > ((EarthquakeMarker) qmarker).threatCircle()) {
							qmarker.setHidden(true);
						}
					}
				}
